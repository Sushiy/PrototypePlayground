...
public class FastIKFabrik : MonoBehaviour
{
    ...
    private void ResolveIK()
    {
        ...
        for (int i = 0; i < positions.Length; i++)
        {
            ...
            {
                bones[i].rotation = target.rotation * Quaternion.Inverse(startTargetRotation) * startBoneRotations[i];
            }
            else
            {
                bones[i].rotation = Quaternion.FromToRotation(startDirectionSucc[i], positions[i + 1] - positions[i]) * startBoneRotations[i];
            }
